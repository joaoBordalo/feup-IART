 ...
 */
public class LeftGaussianFunction implements FuzzySetFunction, Serializable
{
    /**
     ...
     * 
     */
    static int leftGaussianFunctionDefaultNumPoints;
    
	static 
	{
		leftGaussianFunctionDefaultNumPoints = 9;
	}    
    ...
    /**
     * Creates an instance of an LeftGaussianFunction that is used to generate Fuzzysets
    ...
    /**
     * Creates an instance of a LeftGaussianFunction that is used to generate Fuzzysets
     * with a gaussian-shape for its left part. A FuzzySet created will have a 0 value 
     * at its left and a 1 value at its right. When created with this constructor a 
     ...
     * <code>leftGaussianFunctionDefaultNumPoints</code>.
     *
     * @param leftX  the bottom left x value of the gaussian-shaped curve.
     * @param rightX the upper right x value of the gaussian-shaped curve.
     */
    public FuzzySet generateFuzzySet(double leftX, double rightX)
    {
        if(defaultNumPoints < 5) return(generateFuzzySet(leftX, rightX, leftGaussianFunctionDefaultNumPoints));
     ...
     * The left point is actually 4 standard deviations from the right (centre) point of
     * the gaussian curve. The left point and all point to its left are assumed to be zero.
     * <br>
    ...
    public FuzzySet generateFuzzySet(double leftX, double rightX, int numberOfPoints)
    {
        ...
        for(int i=1; i<numPoints-1; i++)
        {
            ...
            x += deltaX;
            membershipValue = Math.pow(Math.E, -((x-rightX)*(x-rightX))/twoSigmaSquared);
    ...
    {
        if(numPoints < 5)               leftGaussianFunctionDefaultNumPoints = 5;
        else                            leftGaussianFunctionDefaultNumPoints = numPoints;
    }
