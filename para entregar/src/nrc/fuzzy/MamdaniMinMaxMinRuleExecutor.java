 ...
 * Mamdani Min inference operator and MaxMin composition. This 
 * method has the rather nice property that the outputs are generated from the
 * conclusions by 'clipping' them at a value that is calculated (DOF -- Degree of 
 * Fullment of the rule) as the minimum (or product etc. depending on current 
 * AntecedentCombine Operator)
...
public class MamdaniMinMaxMinRuleExecutor extends FuzzyRuleExecutor
                                           implements Serializable
{
    ...
    public Object clone() 
    {
      try 
      {
        MamdaniMinMaxMinRuleExecutor re = (MamdaniMinMaxMinRuleExecutor)super.clone();
        re.DOF = this.DOF;
        return re;
      } 
      catch (Exception e) 
     ...
     * Mamdani Min inference operator and Max-Min composition.
        ...
        throws IncompatibleRuleInputsException
    {
        ...
        // now calc actual conclusions using DOF to 'clip' each
        // conclusion fuzzy value
        for (i = 0; i<conclusions.size(); i++)
        {   FuzzyValue conc = conclusions.fuzzyValueAt(i);
            outputs.addFuzzyValue(conc.horizontalIntersection(DOF));
        }
